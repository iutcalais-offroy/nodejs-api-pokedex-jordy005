...
const validateCardsOrThrow = async (cards: unknown) => {
  ...
  if (cards.length !== 10) {
    return { ok: false as const, status: 400, message: "Le deck doit contenir exactement 10 cartes." };
  }

  if (!cards.every((c) => Number.isInteger(c))) {
    return { ok: false as const, status: 400, message: "Les IDs de cartes doivent être des entiers." };
  }

  const unique = Array.from(new Set(cards as number[]));
  if (unique.length !== 10) {
    return { ok: false as const, status: 400, message: "Les IDs de cartes doivent être tous différents." };
  }

  const existingCount = await deckRepository.countExistingCards(unique);
  if (existingCount !== 10) {
    return { ok: false as const, status: 400, message: "Certaines cartes n'existent pas." };
  }
...
export const deckService = {
  ...
  async getById(userId: number, deckIdRaw: string) {
    ...
    if (!deckId) return { ok: false as const, status: 404, message: "Deck introuvable" };

    const deck = await deckRepository.findDeckByIdForUser(deckId, userId);
    if (!deck) return { ok: false as const, status: 404, message: "Deck introuvable" };

    return { ok: true as const, status: 200, deck };
  },

  async update(userId: number, deckIdRaw: string, name: unknown, cards: unknown) {
    const deckId = parseId(deckIdRaw);
    if (!deckId) return { ok: false as const, status: 404, message: "Deck introuvable" };

    const owned = await deckRepository.findDeckByIdForUser(deckId, userId);
    if (!owned) return { ok: false as const, status: 404, message: "Deck introuvable" };

    if (name === undefined && cards === undefined) {
      return { ok: false as const, status: 400, message: "Aucune donnée à modifier" };
    }
    ...
    const updated = await prisma.$transaction(async (tx) => {
      if (name !== undefined) {
        await tx.deck.update({ where: { id: deckId }, data: { name: name.trim() } });
      }
